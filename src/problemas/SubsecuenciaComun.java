...
public class SubsecuenciaComun implements Problema {
     ...
     * 
     * @param longitud1 entero que indica la longitud deseada en la cadena 1
     * @param longitud2 entero que indica la longitud deseada en la cadena 2
     * @throws Exception 
     */
	...
	public String getTipo() {
		return TIPO.SUBSECUENCIA.toString();
	}
	...
	public String execute() {
		...
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= m; j++) {
				if (cadena1.charAt(i - 1) == cadena2.charAt(j - 1)) {
					...
					aux[i][j] = 2; //toma el valor del elemento superior en la matriz
				} else {
					matriz[i][j] = matriz[i][j - 1];
					aux[i][j] = 3; //toma el valor del elemento anterior en la matriz
				}
			}
		}
		...
		int j = m;
		while (i != 0 && j != 0) {
			if (aux[i][j] == 1) { //Coincidencia de caracteres, continua en diagonal
	 ...
	 * @param n tamaño de la cadena 1
	 * @param m tamaño de la cadena 2
	 */
